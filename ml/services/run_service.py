...
def run_ml_experiment(user: User, dataset: Dataset, pipeline: PreprocessingPipeline | None, model, split_config: dict, used_parameters: dict) -> dict:
    """
    ...
    try:
        ...
        if pipeline:
            ...
            if not result:
                return {'error': 'Brak danych w pipeline. Skonfiguruj preprocessing.', 'status': 'Failed'}
        ...
        if not ModelClass:
            return {'error': f"Model '{model.name}' nie jest obsługiwany.", 'status': 'Failed'}
...
def _extract_metrics(evaluation: dict) -> dict:
    ...
    metrics = {}
    desired_keys = [
        'accuracy', 'f1', 'mean_absolute_error', 'mean_squared_error', 'r2_score',
