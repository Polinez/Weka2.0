...
def extract_parameters_from_post(post_data: dict, model: MLModel | None) -> dict:
    ...
    try:
        common["test_size"] = (
            convert_param_value(post_data.get("common_test_size"), "float") or 0.2
        )
        common["random_state"] = (
            convert_param_value(post_data.get("common_random_state"), "int") or 42
        )
...
def prepare_model_params_for_display(
    session_params: dict, common_defaults: dict, selected_model: MLModel | None
) -> tuple[list, list]:
    """
