...
def handle_preprocessing_request(
    pipeline: PreprocessingPipeline, feature: str, operation: str, post_data: dict
) -> tuple[str | None, str]:
    """
    ...
    if operation == "scale" and feature == target_col:
        return (
            f"Nie można skalować kolumny docelowej '{feature}'. Zmieniłoby to wartości predykcji i utrudniło interpretację wyników.",
            "",
        )
    ...
    type_map = {
        "impute": (
            "Imputation",
            lambda: {"method": post_data.get("imputation_method", "mean")},
        ),
        "encode": (
            "Encoding",
            lambda: {"method": post_data.get("encoding_method", "label_encoder")},
        ),
        "scale": (
            "Scaling",
            lambda: {"method": post_data.get("scaling_method", "standardization")},
        ),
...
def get_or_create_active_pipeline(
    dataset: Dataset, split_config: dict
...
def _apply_imputation(
    df_train: pd.DataFrame, df_test: pd.DataFrame, col: str, params: dict
):
    method = params.get("method", "mean")
    strategy = (
        "mean"
        if method == "mean"
        else ("median" if method == "median" else "most_frequent")
    )
...
def _apply_encoding(
    ...
    if method == "label_encoder":
        ...
        ohe = OneHotEncoder(
            handle_unknown="ignore", drop=None, sparse_output=False, dtype=int
        )
